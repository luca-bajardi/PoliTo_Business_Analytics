...
sum(is.na(rating))
```
Possiamo anche osservare la distribuzione dei rating dei diversi film ed osservare il valore medio.
```{r hist}
hist(rating)
mean(rating)
```
...
```
Andando ad osservare tutti i possibili modelli, quello migliore si ottiene utilizzando solamente la variabile "Prevratings" perchè probabilmente le altre variabili non aggiungono troppi miglioramenti alle informazioni che abbiamo già ricavato.
...
summary(regfit.bwd)
```
\textcolor{red}{Se proviamo ad osservare i coefficienti per uno specifico numero di variabili, vediamo che non coincidono esattamente però i valori sono comunque molto vicini. }
...
In conclusione possiamo dire che il modello migliore con un MSE di 3.660313 è il modello ridge con $\lambda = 2.158988$ e coefficienti:
```{r conclusioni}
ridge.coef
```
Da questo possiamo capire che il network CBC di default ha un rating più basso di ABN, ma più alto di BBS. La presenza di un film basato su una storia vera fa aumentare il rating, come lo fa aumentare la presenza di star. Come nell'analisi iniziale anche il rating del programma precedente fa aumentare il rating del programma serale, ma come si è visto l'uso di questa informazione da sola ha una previsione meno precisa.

```{r conclusioni2}
mean(abs(ridge.pred-y.test))
```

Il modello migliore che abbiamo trovato è buono, ma non è ottimo perché l'errore assoluto medio è 1.52 quindi implica che (se ogni punto in meno di rating si perdono 1 milione di dollari come nella parte B del Business Case) c'è il rischio di guadagnare un milione e mezzo di dollari in meno rispetto alla previsione.