...
L'obiettivo è quello di capire quali sono le variabili e come queste interagiscono, nell'influenzare gli ascolti per un determinato programma. Gli ascolti vengono espressi attraverso un valore di rating, nello specifico, ogni punto di rating rappresenta 921000 famiglie americane che stanno guardando il nostro programma.
Formuliamo diversi modelli di regressione per capire quali sono le variabili migliori da utilizzare per prevedere quale sarà il reting del film al quale siamo interessati.
...
Inizialmente proviamo a formulare dei modelli con un'unica variabile, quella del rating del programma precedente (è infatti l'unica variabile continua), anche se ovviamente non ci permette di avere una spiegazione completa. Per questo vorremmo introdurre nei modelli, altre variabili.
Per capire quante e quali variabili andare ad utilizzare, applichiamo il metodo esaustivo, il metodo backward e quello forward, in modo da stabilire qual sia il modello migliore.
