...
library(leaps)
CBC=CBC[, -c(1,5, 8)]
regfit.full=regsubsets(rating~.,CBC,nvmax=14)
summary(regfit.full)
...
Poichè stiamo confrontando modelli con un numero diverso di parametri piuttosto che
...
regfit.best=regsubsets(rating~.,data=CBC[train,],nvmax=12)
test.mat=model.matrix(rating~.,data=CBC[test,])
...
for(i in 1:12){
  ...
  val.errors[i]=mean((CBC$rating[test]-pred)^2)
}
val.errors
...
regfit.fwd=regsubsets(rating~.,data=CBC,nvmax=12,method="forward")
summary(regfit.fwd)
regfit.bwd=regsubsets(rating~.,data=CBC,nvmax=12,method="backward")
summary(regfit.bwd)
```
...
for(j in 1:k){
  best.fit=regsubsets(rating~.,data=CBC[folds!=j,],nvmax=12)
  for(i in 1:12){
    pred=predict(best.fit,CBC[folds==j,],id=i)
...
reg.best=regsubsets(rating~.,data=CBC, nvmax=12)
```
...
La funzione `glmnet` implemnta sia i due casi estremi di regressione ridge e lasso, sia regressioni intermedie a seconda del valore di $\alpha$ che fissiamo.
```{r ridge}
