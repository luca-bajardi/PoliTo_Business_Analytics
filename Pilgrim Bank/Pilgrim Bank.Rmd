...
Per provare a far aumentare il valore dell'$R^2$, possiamo utilizzare le informazioni a nostra disposizione sull'anno precedente, quelle che abbiamo utilizzato nei modelli precedenti.
...
IncomeZero = ifelse(is.na(Income),0,Income)
IncomeGiven = ifelse(is.na(Income),0,1)
mod3 = lm(Profit0 ~ Profit9+Online9+AgeZero+AgeGiven+IncomeZero+IncomeGiven+Tenure+District1100+District1200, data=data)
summary(mod3)
mod4 = lm(Profit0 ~ Profit9+Online9+Tenure+District1100+District1200)
summary(mod4)
```
Osserviamo che abbiamo un numero diverso di valori mancanti 
```{r na}
sum(is.na(Online0))
sum(is.na(Profit0))
...
summary(glm.out)
#molto veloce,i coefficienti sono piccolini, ma vanno dentro l'esponenziale
#le diasgnostiche sono un po' diverse, ma non cambia il senso generale
hist(glm.out$fitted.values,nclass=50)
#continuiamo ad avere le due mode molto distanti e 
#dei casi un po' dubbi nel mezzo
plot(mod3$fitted, glm.out$fitted, xlab="Predicted OLS", ylab="Predicted logit")
#le prestazioni sono davvero meglio?
plot(profit9, profit0) #buttare fuori le anomalie
...
dtab=subset(dtab, subset=X0Profit<= -30000)
#ci sono altre informazioni che ci dicono quanto è profittevole un cliente
#due cose che assolutamente vanno fatte
#eliminare gli otliers
#introdurre un elemento di serie temporali
```
