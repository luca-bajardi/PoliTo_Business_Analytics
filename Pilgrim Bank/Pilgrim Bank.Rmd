...
```{r decisiontree}
library(tree)
setup<-tree.control(52792, mincut = 2, minsize = 6, mindev = 0.001)
tree.Pilgrim = tree(Retain ~ Profit9+Online9+AgeZero+AgeGiven+IncomeZero+IncomeGiven+Tenure+District1100+District1200, data=data_balanced_over, control = setup)
summary(tree.Pilgrim)
```
...
target.train = data_balanced_over[train,]$Retain
target.test = df.test$Retain
setup<-tree.control(52792-length(target.test),mincut = 2, minsize = 6, mindev = 0.001)
tree.Pilgrim = tree(Retain ~ Profit9+Online9+AgeZero+AgeGiven+IncomeZero+IncomeGiven+Tenure+District1100+District1200, data=data_balanced_over, control = setup, subset = train)
tree.pred = predict(tree.Pilgrim, df.test, type="class", probability=TRUE)
#table.res = table(tree.pred,target.test)
#table.res
#accuracy = (table.res[1,1]+table.res[2,2])/sum(table.res)
#accuracy
library(caret)
cf = confusionMatrix(data = tree.pred, reference = target.test, mode = "prec_recall")
cf
precision <- posPredValue(tree.pred,target.test, positive="1")
recall <- sensitivity(tree.pred,target.test, positive="1")
...
cf
precision <- posPredValue(logit.pred,target.test, positive="1")
recall <- sensitivity(logit.pred,target.test, positive="1")
...
#fit=train(Retain ~ Profit9+Online9+AgeZero+AgeGiven+IncomeZero+IncomeGiven+Tenure+District1100+District1200,
#          method="knn",
...
```{r knn}
library(class)
knn_pred <- knn(df.train,df.test,cl=target.train,k=2)
cf = confusionMatrix(data = pr, reference = target.test, mode = "prec_recall")
cf
precision <- posPredValue(pr,target.test, positive="1")
recall <- sensitivity(pr,target.test, positive="1")
specificity=specificity(pr,target.test, positive="1")
cat('Specificity:',specificity, '\n')
```
```{r roc, message=FALSE}
library(pROC)
p=predict(tree.Pilgrim, df.train)
...
#legend("bottomright", legend = c("Logistic Regression", "Decision Tree", "KNN"), col=c("green", "blue", "red"))
```
